...
class FunctionCallNode final : public IMaybeStatementASTNode {
  public:
    static constexpr ASTNodeKind kKind = ASTNodeKind::FunctionCall;

    FunctionCallNode(Parser::ParseContext& Context, Lexer::TokenSeparator* Token, IASTNode* ExpressionIn);
    FunctionCallNode(Lexer::IToken* NearbyTokenIn, IASTNode* ExpressionIn, std::vector<IASTNode*> ArgumentsIn)
  ...
  private:
    ...
    void GenerateCode(Bytecode::BytecodeContext& Context, bool InChain);
    static bool FunctionCallNodeOnLeftmostSide(DotVariableNode* Node);
